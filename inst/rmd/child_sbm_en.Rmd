...
if (is_bipartite) {
  example_group <- shinySbm:::round_proportion(R$sbm$blockProp$row)[[R$sbm$nbBlocks[[1]]]] * 100
} else {
  example_group <- shinySbm:::round_proportion(R$sbm$blockProp)[[R$sbm$nbBlocks[[1]]]] * 100
}
...
if (R$sbm$modelName == "bernoulli") {
  connect_paraph <- paste0(
    ...
    col_group_names[[ex_index]], " in columns. Then there is a probability of ", example_val," that node A is connected to node B."
  )
