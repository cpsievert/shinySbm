...
if(R$sbm$ICL != max(R$sbm$storedModels$ICL)){
  entrop_cur <- paste0("The most likely model has an ILC of ",
...
if(is_bipartite){
  c(col_group_names,
    ...
    t %>% 
    as.data.frame() %>% 
    smalltable(title = 'Proportion of groups in columns')  %>% 
  ...
  c(row_group_names,
    ...
    t %>%
    as.data.frame() %>%
    smalltable(title = 'Proportion of groups in lines')  %>% 
    flextable::flextable_to_rmd()
}else{
  c(row_group_names,
    round_prop(R$sbm$blockProp$row) %>% 
    ...
    t %>% 
    as.data.frame() %>% 
    smalltable(title = 'Proportion of groups')  %>% 
    flextable::flextable_to_rmd()
}
```
<br>
Here, for example, `r round_prop(R$sbm$blockProp$row)[[R$sbm$nbBlocks[[1]]]]*100`\% of the `r R$upload$labels$row` are in the `r col_group_names[[R$sbm$nbBlocks[[1]]]]` group. 
...
if(R$sbm$modelName == "gaussian"){
  connect_table <- flextable::add_body_row(connect_table,top = F,
      values = c('Variance',as.character(round(R$sbm$connectParam$var[[1]],3)),""),
       colwidths = c(1,1,R$sbm$nbBlocks[[2]]-1))
}
...
```
<br> 
The values in this table are the connectivity parameters of the sbm model (`r modelName`). <br>
```{r echo=FALSE,results='asis'}
example_val <- round(R$sbm$connectParam$mean[1,2],nb_digits)
if(R$sbm$modelName == "bernoulli"){
  connect_paraph <- paste0("For example, if we take a node A in the group ",
         row_group_names[[1]]," in rows and a node B in the group ",
         col_group_names[[2]]," in columns. Then node A ","\ue0",
         "a probability ",example_val," of being connected to node B.")
}else if(R$sbm$modelName == "poisson"){
  connect_paraph <- paste0("For example, nodes in group ",
         row_group_names[[1]]," in rows and group nodes ",
         col_group_names[[2]]," in columns have an average connection value of ",
